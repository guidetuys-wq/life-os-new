...
import { 
    ...
    doc, increment, writeBatch, serverTimestamp // <--- [UPDATE] Pakai writeBatch & serverTimestamp
...
export default function FinancePage() {
    ...
    // 1. Fetch Global Stats & Transactions
    useEffect(() => {
        if (!user) return;
        ...
        // Listen Transactions (History 50)
    ...
    // [FIX 1] Handle Save dengan BATCH (Atomic)
    const handleSave = async (e) => {
        ...
        try {
            ...
            // A. Siapkan Referensi Dokumen Baru
            const newTransRef = doc(collection(db, 'artifacts', appId, 'users', user.uid, 'transactions'));
            
            // B. Operation 1: Simpan Transaksi
            ...
            // C. Operation 2: Update Global Stats (Increment)
            const statsRef = doc(db, 'artifacts', appId, 'users', user.uid, 'stats', 'finance');
            const amountVal = amt; // Positif karena menambah data

            batch.set(statsRef, {
                [formData.type]: increment(amountVal),
                balance: increment(formData.type === 'income' ? amountVal : -amountVal)
            }, { merge: true });

            // D. Commit Batch (Kirim Semua Sekaligus)
            await batch.commit();
            ...
            console.error(error);
            toast.error('Gagal menyimpan (Database Sync Error)'); 
        }
    };

    // [FIX 1] Handle Delete dengan BATCH (Atomic)
    const handleDelete = async (t) => {
        if(confirm('Hapus transaksi ini? Saldo akan disesuaikan otomatis.')) {
            try {
                const batch = writeBatch(db);

                // A. Operation 1: Hapus Dokumen
                const transRef = doc(db, 'artifacts', appId, 'users', user.uid, 'transactions', t.id);
                batch.delete(transRef);

                // B. Operation 2: Revert Global Stats (Decrement/Balik Arah)
                const statsRef = doc(db, 'artifacts', appId, 'users', user.uid, 'stats', 'finance');
                const amountVal = Math.abs(t.amount) * -1; // Negatif karena menghapus data

                batch.set(statsRef, {
                    [t.type]: increment(amountVal),
                    balance: increment(t.type === 'income' ? amountVal : -amountVal)
                }, { merge: true });
            ...
            <div className="relative overflow-hidden rounded-[2rem] p-8 mb-8 border border-white/10 shadow-2xl group">
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-end gap-6">
                    <div>
                        ...
                        {/* [FIX 2] Responsive Font Size agar tidak pecah di HP */}
                        <h2 className="text-3xl md:text-5xl font-mono font-bold text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-400 truncate">
                    ...
                    <div key={t.id} className="group flex justify-between items-center p-4 bg-slate-900/40 hover:bg-slate-800/60 rounded-2xl border border-slate-800 hover:border-slate-600 transition-all cursor-default">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                {/* [FIX 3] Fallback Category jika data lama tidak punya kategori */}

...
import { refineTaskAction } from '@/app/actions/ai';
...
export default function InboxWidget() {
    ...
    useEffect(() => {
        ...
        // [FIX] Filter di client-side: Hanya tampilkan task TANPA projectId
        ...
        const unsubTasks = onSnapshot(qTask, (snap) => {
            ...
            // Filter: Belum selesai DAN (projectId kosong ATAU null)
            const inboxTasks = rawTasks.filter(t => 
                !t.completed && (!t.projectId || t.projectId === '')
            );
    ...
    const completeTask = async (id, text) => {
        try {
            // Update jadi completed: true
            await updateDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'tasks', id), {
                completed: true,
                completedAt: serverTimestamp() // Berguna untuk log/history nanti
            });
    ...
    const handleRefine = async (task) => {
        ...
        const refinedText = await refineTaskAction(task.text);
